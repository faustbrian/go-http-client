...
`go-http-client` is a policy layer for typed outbound HTTP integrations. It is
...
safe `slog`/`go-log` hooks, standard OpenTelemetry/go-telemetry adapter ports,
...
`go-circuit-breaker` for all state and half-open probe control:
