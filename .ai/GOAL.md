...
- first-party `go-circuit-breaker` integration through a narrow interface,
...
- in-memory reference backend plus optional `go-cache` or application-provided
...
- `slog` hooks and optional `go-log` integration without mandatory logging
- OpenTelemetry tracing and metrics through optional `go-telemetry` adapters
...
`go-circuit-breaker` owns generic breaker state and policy. `go-http-client`
...
`go-wire` owns JSON, XML, SOAP, YAML, TOML, and MessagePack codecs.
`go-http-client` owns codec invocation, media-type validation, body limits,
response ownership, and decode error context. `go-cache` owns reusable cache
backends while this module owns HTTP cache semantics and cache-key safety.
`go-queue` owns durable delay and replay; this module owns only bounded
...
- no durable request scheduling or delayed-job persistence; use `go-queue`
...
4. Implement retry, delay, rate-limit, and `go-circuit-breaker` composition.
