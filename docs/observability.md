...
## OpenTelemetry and go-telemetry

The ports use standard `context.Context` and `http.Header`, so adapters do not
require a core dependency on an exporter or SDK. A `go-telemetry` runtime
...
exporter, sampling, and shutdown ownership with `go-telemetry`.
...
never logs URL, headers, payloads, scope values, or error text. `go-log`
constructors return a standard `*slog.Logger`, so they plug into this adapter
directly without adding a mandatory `go-log` dependency:
