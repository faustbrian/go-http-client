...
func TestPolicyProfilesRejectUnknownAndInvalidOverrides(t *testing.T) {
	...
	//nolint:staticcheck // This boundary test verifies nil is rejected safely.
	if _, err := WithPolicyOverrides(nil, PolicyOverrides{}); !errors.Is(err, ErrInvalidPolicyProfile) {
	...
	//nolint:staticcheck // This boundary test verifies nil is read safely.
	if _, ok := ResolvedPolicyFromContext(nil); ok {
		t.Fatal("nil context returned resolved policy")
	}
	//nolint:staticcheck // This boundary test verifies nil is read safely.
	if overrides := requestPolicyOverrides(nil); overrides.OperationTimeout != nil {
