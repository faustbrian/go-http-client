...
audit of `go-http-client`, then implement every justified correction required
...
- `go-circuit-breaker` state, rejection, half-open probe, HTTP classification,
  retry, limiter, cache, and telemetry ordering
- proof that breaker state is owned only by `go-circuit-breaker`
